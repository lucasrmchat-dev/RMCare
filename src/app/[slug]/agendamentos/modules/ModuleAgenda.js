...
export default function ModuleAgenda() {   
  ...
  // Pega o dia da semana da data (0 = Dom, 6 = Sáb)
  ...
  // Filtra as regras cadastradas para o Profissional e o Dia da Semana selecionado
  const regrasDoDia = agenda.regras?.filter(r => 
    r.servico_id === selectedSrv?.id && 
    r.dias_semana.includes(diaSelecionado) &&
    r.ativo !== false
  );

  // Função Matemática para calcular Slots
  const gerarSlotsParaTurno = (startStr, endStr, duracaoMinutos, customLastAllowed) => {
    ...
    // Regra 2: Cálculo do último horário = Término do Turno - Duração da Consulta
    let limit = new Date(end.getTime() - (duracaoMinutos * 60000)); 
    
    // Sobrescreve com o customizado do painel, caso o admin tenha configurado
  ...
  // ---------------------------------------------------------
  // GERAÇÃO DOS HORÁRIOS (O MOTOR DO CALENDÁRIO)
  // ---------------------------------------------------------
  let slotsRender = [];
  ...
  if (formData.data_agendamento) {
    ...
    if (regrasDoDia && regrasDoDia.length > 0) {
      // REGRA PERSONALIZADA (Filtra se o tipo bate com a regra)
      const regrasValidas = regrasDoDia.filter(r => {
        ...
        const currentString = formData.tipo_servico === "Exame" 
          ? `Exame (${formData.subtipo_exame})` 
          : `Consulta ${formData.modalidade || "Particular"}`;
        
        return r.tipos_permitidos.some(tp => 
          tp.toLowerCase().includes(formData.tipo_servico.toLowerCase()) || 
          currentString.toLowerCase().includes(tp.toLowerCase()) || 
          tp.toLowerCase().includes(formData.subtipo_exame?.toLowerCase())
        );
      });

      // Roda por cada turno válido no dia (ex: Regra da Manhã + Regra da Tarde)
      regrasValidas.forEach(r => {
      ...
      // REGRA DE FALLBACK (Geral / Padrão)
      let duracaoFallback = 40; // Particular / Primeira Consulta
    ...
    // Ordena, remove duplicatas e aplica lógica de Filtro (Bloqueados e Sequenciais)
    slotsGerados.sort();
    ...
    slotsRender = slotsGerados.map(h => {
      // Bloqueia horários passados caso seja o dia de hoje
      const isPastTime = formData.data_agendamento === helpers.getToday() && new Date().setHours(...h.split(':'), 0, 0) <= Date.now() + 1800000; // 30 min de respiro
      
      // Verifica no BD se já existe paciente ou bloqueio manual neste horário
      const isOccupied = agenda.ocupados.includes(h) || isPastTime;
      
      let off = isOccupied;
      
      // REGRA: Trava de Ocupação Sequencial
      if (ocupacaoSeqAtiva) {
        if (!isOccupied) {
          if (encontrouPrimeiroLivre) {
            // Já encontramos um slot livre antes, então bloqueamos todos os futuros
            off = true;
          } else {
            // Este é o primeiro slot livre da fila! Habilitamos e marcamos a flag.
  ...
  // Define se um dia inteiro deve ser bloqueado na renderização do mês
  const isDiaPermitidoPelasRegras = (dataStr) => {
    const diaWeek = getDiaSemana(dataStr);
    const regrasParaODia = agenda.regras?.filter(r => r.servico_id === selectedSrv?.id && r.dias_semana.includes(diaWeek) && r.ativo !== false);
    
    // Se o médico tem regras criadas na base, mas nenhuma é pra este dia, o dia fica indisponível.
    // Se ele não tem regra nenhuma criada, a agenda roda no modo Livre (Fallback).
    const temQualquerRegra = agenda.regras?.some(r => r.servico_id === selectedSrv?.id && r.ativo !== false);
              ...
              <div>                 
                ...
                {slotsRender.length === 0 ? (
                  <div className="text-center p-6 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800">
                    <p className="text-sm text-zinc-500">O profissional não realiza este tipo de atendimento nesta data.</p>
                  </div>
                ) : (
