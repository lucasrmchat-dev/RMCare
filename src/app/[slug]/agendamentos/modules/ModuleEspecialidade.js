...
export default function ModuleEspecialidade() {   
  ...
  // Extrai as especialidades separando as que contêm vírgula e remove duplicatas
        ...
        <div className="w-full">           
          ...
          {!formData.especialidade ? (
              ...
              {especialidades.length > 0 ? especialidades.map(esp => (                   
                <button 
                  ...
                  onClick={() => {
                    ...
                    setValue("medico_profissional", "");
                    setValue("tipo_servico", "Consulta"); 
                  }} 
            ...
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">                 
                {servicosDB
                  // Verifica se a especialidade selecionada está dentro do array separado por vírgulas do médico
                  .filter(s => s.especialidade && s.especialidade.split(',').map(e => e.trim()).includes(formData.especialidade))
                  .map(m => (                   
                  <button 
                    key={m.id} 
                    onClick={() => setValue("medico_profissional", m.nome)} 
