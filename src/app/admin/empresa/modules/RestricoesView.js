...
import { CalendarDays, CheckCircle2, Plus, Clock, Clock3, Trash2, Building, User, Pencil, X, LayoutGrid, List } from "lucide-react"; 
                            ...
                            <div className="bg-white/80 dark:bg-[#0c0c0e]/80 backdrop-blur-2xl border border-zinc-200/80 dark:border-white/10 rounded-[2rem] shadow-sm p-6 md:p-8 space-y-8">
                                {editingId && (
                                    <div className="flex items-center justify-between rounded-2xl bg-blue-50/70 dark:bg-blue-950/30 border border-blue-200/60 dark:border-blue-900/40 px-4 py-3 text-xs">
                                        <div>
                                            <strong className="text-blue-950 dark:text-blue-300">Editando horário existente</strong>
                                            <p className="text-blue-700 dark:text-blue-400 mt-0.5">Altere os campos abaixo e salve para substituir a configuração anterior.</p>
                                        </div>
                                ...
                                <section>                       
                                    ...
                                    <div className="grid md:grid-cols-2 gap-3 mb-4">                         
                                        <button onClick={() => setTipoRegra("geral")} className={`p-4 rounded-2xl border-2 transition-all flex flex-col gap-2 text-left ${tipoRegra === "geral" ? "border-zinc-900 bg-zinc-50 dark:bg-zinc-900/70 dark:border-white shadow-sm" : "border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/30"}`}>                           
                                            ...
                                            <div>                             
                                                ...
                                                <span className="block text-xs text-zinc-500 mt-0.5">Aplica-se a todos os agendamentos da clínica.</span>                           
                                            </div>                         
                                        </button>                                                  
                                        <button onClick={() => setTipoRegra("especifica")} className={`p-4 rounded-2xl border-2 transition-all flex flex-col gap-2 text-left ${tipoRegra === "especifica" ? "border-zinc-900 bg-zinc-50 dark:bg-zinc-900/70 dark:border-white shadow-sm" : "border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/30"}`}>                           
                                            <User size={20} className={tipoRegra === "especifica" ? "text-zinc-900 dark:text-white" : "text-zinc-400"} />                           
                                            <div>                             
                                                <span className="block font-bold text-zinc-900 dark:text-white text-sm">Colaborador / Profissional</span>                             
                                                <span className="block text-xs text-zinc-500 mt-0.5">Exclusivo para um médico ou exame específico.</span>                           
                                            </div>                         
                                        </button>                       
                                    </div>                       
                        ...
                        <motion.div key="lista" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={spring}>               
                            {regras.length === 0 ? (                 
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">                   
                                    {regras.map((regra) => {                     
                                        const profissional = regra.servico_id ? (servicosOptions.find(s => s.value === regra.servico_id) || { label: "Profissional Desconhecido" }) : { label: "Geral da Clínica" };                                          
                                            ...
                                            <motion.div key={regra.id} layout initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="bg-white/80 dark:bg-[#0c0c0e]/80 backdrop-blur-2xl border border-zinc-200/80 dark:border-white/10 p-5 rounded-2xl flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow group">                         
                                                <div className="space-y-3">                           
                                                    <div className="flex items-start justify-between gap-3">                             
                                                        <div>                               
                                                            <h4 className="text-base font-bold text-zinc-950 dark:text-white leading-tight flex items-center gap-2">                                   
                                                                {!regra.servico_id && <Building size={15} className="text-blue-500" />}                                   
                                                                {profissional.label}                               
                                                            </h4>                               
                                                            <p className="text-[11px] font-bold text-zinc-400 mt-1 uppercase tracking-widest flex items-center gap-1.5">                                 
                                                                <CalendarDays size={13} /> {diasNomes}                               
                                                            </p>                             
                                                        </div>                                                          
                                      ...
                                      {regras.map((regra) => {
                                        ...
                                        return (
                                          <tr key={regra.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition-colors">
                                            <td className="p-3.5 font-bold text-zinc-950 dark:text-white">{profissional.label}</td>
                                            <td className="p-3.5 font-semibold text-zinc-700 dark:text-zinc-300">{diasNomes}</td>
