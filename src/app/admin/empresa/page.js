    ...
    return (         
        <div className="h-screen w-screen bg-[#F4F4F5] dark:bg-black flex flex-col font-sans overflow-hidden text-zinc-900 dark:text-white selection:bg-zinc-900 selection:text-white">             
            ...
            <div className="flex flex-1 overflow-hidden relative">                 
                <aside className={`absolute md:relative z-40 h-full w-[260px] bg-white dark:bg-[#0a0a0a] border-r border-zinc-200/60 dark:border-zinc-800 flex flex-col p-4 transition-transform duration-500 ease-out ${isMobileMenuOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full md:translate-x-0'}`}>                     
                    <div className="mb-8 px-4 pt-4">                         
                        <h2 className="text-[11px] font-semibold text-zinc-400 dark:text-zinc-500 mb-1">RMCare</h2>                         
                    ...
                    <LayoutGroup>                         
                        <nav className="flex flex-col gap-1 flex-1 overflow-y-auto custom-scrollbar">                             
                            <SidebarItem id="agenda" icon={CalendarDays} label="Agenda de pacientes" activeView={activeView} onClick={handleMenuClick} />                             
                            ...
                            <SidebarItem id="integracoes" icon={Link2} label="Integrações" activeView={activeView} onClick={handleMenuClick} />                             
                ...
                <main className="flex-1 flex flex-col relative overflow-hidden bg-[#F4F4F5] dark:bg-black md:rounded-tl-[2rem] border-t border-l border-zinc-200/50 dark:border-zinc-800 shadow-[-10px_-10px_30px_rgba(0,0,0,0.02)]">                     
                    <AnimatePresence mode="wait">                         
                        ...
                        {activeView === "integracoes" && ( <IntegracoesView bloqueios={bloqueios} servicos={servicos} fetchBloqueios={fetchBloqueios} fetchServicos={fetchServicos} showToast={showToast} /> )}                         
                    </AnimatePresence>                 
                </main>             
            </div>             
