...
import { 
    CalendarDays, Zap, X, Users, FileQuestion, Menu, Lock, CheckCircle2, AlertCircle, LayoutTemplate, CreditCard 
} from "lucide-react";

import Navbar from "@/components/Navbar";
import { SidebarItem, spring } from "./components/SharedUI";
import { 
    fetchAdminBloqueios, fetchAdminAgendamentos, fetchAdminServicos, 
    fetchAdminPerguntas, fetchAdminRegras 
            ...
            <div className="flex flex-1 pt-[72px] overflow-hidden relative">
                <aside className={`absolute md:relative z-40 h-full w-[260px] bg-white border-r border-zinc-200/60 flex flex-col p-4 transition-transform duration-500 cubic-bezier(0.16, 1, 0.3, 1) ${isMobileMenuOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full md:translate-x-0'}`}>
                    ...
                    <LayoutGroup>
                        <nav className="flex flex-col gap-1 flex-1 overflow-y-auto custom-scrollbar">
                            ...
                            <SidebarItem id="bloqueios" icon={Lock} label="Regras & Bloqueios" activeView={activeView} onClick={handleMenuClick} />
                            <SidebarItem id="equipe" icon={Users} label="Equipe Clínica" activeView={activeView} onClick={handleMenuClick} />
                            <SidebarItem id="triagem" icon={FileQuestion} label="Motor de Triagem" activeView={activeView} onClick={handleMenuClick} />
                            <SidebarItem id="personalizacao" icon={LayoutTemplate} label="Personalização" activeView={activeView} onClick={handleMenuClick} />
                            <SidebarItem id="integracoes" icon={CreditCard} label="Integrações & Pagamentos" activeView={activeView} onClick={handleMenuClick} />
                ...
                <main className="flex-1 flex flex-col relative overflow-hidden bg-[#F4F4F5] md:rounded-tl-[2rem] border-t border-l border-zinc-200/50 shadow-[-10px_-10px_30px_rgba(0,0,0,0.02)]">
                    <AnimatePresence mode="wait">
                        {activeView === "agenda" && ( <AgendaView agendamentos={agendamentos} bloqueios={bloqueios} servicos={servicos} /> )}
                        {activeView === "bloqueios" && ( <RestricoesView regras={regras} fetchRegras={fetchRegras} servicosOptions={servicosOptions} showToast={showToast} /> )}
                        ...
                        {activeView === "personalizacao" && ( <PersonalizacaoView showToast={showToast} /> )}
